...
def parallel_4_bar(config=False,braced=False,braced2=False):
    ...
    if braced2: graph.add_edge(p3,p1)
    return graph
...
def jansen_walker(config=False,braced=False):
    graph = nx.Graph()
    set_id=count()
    p1 = Point(0,6,0) if config else next(set_id)
    p2 = Point(4,10,0) if config else next(set_id)
    p3 = Point(9,6,0) if config else next(set_id)
    p4 = Point(5,3,0) if config else next(set_id)
    p5 = Point(3,0,0) if config else next(set_id)
    p6 = Point(1,3,0) if config else next(set_id)
    p7 = Point(4,6,0) if config else next(set_id)
